...
public class ReportVaultService : IReportVaultService
{
    ...
    public async Task FinalizeAsync(FinalizeRequest request, CancellationToken cancellationToken)
    {
        ...
        refreshedReport.LastModifiedBy = request.User;
        _repository.UpsertReport(refreshedReport);

        var finalHash = _hashService.ComputeHash(finalPath);
        AppendAudit(refreshedReport.ReportId, request.User, "Finalize", new { finalPath, finalHash, finalizationMode });
    }
